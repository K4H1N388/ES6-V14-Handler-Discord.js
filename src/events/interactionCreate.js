...
export default (client, db) => {
    client.on("interactionCreate", async interaction => { // The event where requests will be answered
        ...
        } else if (interaction.isButton()) ButtonHandler(interaction, db) // If the request is a button
        else if (interaction.isModalSubmit()) ModalHandler(interaction, db) // If the request is a modal
        else if (interaction.isAutocomplete()) AutocompleteHandler(interaction, db) // If the request is an autocomplete
        else if (interaction.isStringSelectMenu()) StringSelectMenuHandler(interaction, db) // If the request is a string select menu
        else if (interaction.isChannelSelectMenu()) ChannelSelectMenuHandler(interaction, db) // If the request is a channel select menu
    })
}